...
import { Sparkles, Mail, Lock, User, Shield, Layers, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] flex items-center justify-center overflow-hidden px-4 py-12 select-none bg-slate-950/5 dark:bg-black/0">
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        ...
        <div className="absolute top-[-20%] right-[-20%] w-[70%] h-[70%] bg-indigo-500/10 rounded-full blur-[120px] dark:bg-indigo-600/15" />
        <div className="absolute bottom-[-20%] left-[-20%] w-[70%] h-[70%] bg-pink-500/10 rounded-full blur-[120px] dark:bg-pink-600/15" />
        ...
        <motion.div
          ...
          className="absolute bottom-[30%] left-[25%] w-80 h-80 bg-pink-500/10 rounded-full blur-[80px]"
        />
      </div>
      ...
      >
        ...
        <motion.div variants={itemVariants} className="text-center mb-8 space-y-2">
          ...
          <p className="text-xs font-semibold text-slate-600 dark:text-slate-300 max-w-xs mx-auto">
            Join the premium portal for seamless committee operations and governance audits.
          </p>
        </motion.div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <motion.div variants={itemVariants} className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">Full Name</label>
            ...
            {errors.name && <span className="text-xs text-rose-600 dark:text-rose-400 font-bold block">{errors.name}</span>}
          </motion.div>

          {/* Email Input */}
          <motion.div variants={itemVariants} className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">Email Address</label>
            <div className="relative">
          ...
          {/* Password Input */}
          <motion.div variants={itemVariants} className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">Password</label>
            <div className="relative">
          ...
          {/* Role Select Input */}
          <motion.div variants={itemVariants} className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider flex items-center gap-1">
              <Shield size={12} className="text-indigo-600 dark:text-indigo-400" /> Account Assignment Role
            </label>
            <select
              value={role}
              onChange={(e) => setRole(e.target.value)}
              className="w-full px-3 py-2.5 bg-slate-100/70 dark:bg-white/5 border border-slate-200 dark:border-white/15 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/40 text-slate-800 dark:text-slate-200 transition-all outline-none font-semibold"
            >
              <option value="member" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Staff Member</option>
              <option value="committee_head" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Committee Head</option>
              <option value="admin" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">System Admin</option>
            </select>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="pt-2">
            ...
            >
              {isSubmitting ? "Registering..." : <>Sign Up <ArrowRight size={13} /></>}
            </button>
          </motion.div>
        </form>
        ...
        <motion.p variants={itemVariants} className="mt-6 text-center text-xs font-semibold text-slate-600 dark:text-slate-400">
          ...
          <Link href="/login" className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 hover:underline font-bold">
            Sign in
          </Link>
        </motion.p>
      </motion.div>
